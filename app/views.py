...
# Rota para abrir a página de início do Rail Trail
@app.route('/rail_trail_open', methods=['GET'])
def rail_trail_open():
    return render_template('rail_trail_open.html')
...
def process_data():
    try:
        # Carregar o arquivo de feedbacks
        FEEDBACK_FOLDER = os.path.join(os.getcwd(), 'feedbacks')
        feedbacks_df = pd.read_excel(os.path.join(FEEDBACK_FOLDER, 'feedbacks.xlsx'))

        # Filtrar os dados para o usuário logado e a data atual
        if 'logged_user' in session:
            logged_username = session['logged_user']
            feedbacks_df = feedbacks_df[feedbacks_df['User'] == logged_username]
            feedbacks_df['Data e Hora'] = pd.to_datetime(feedbacks_df['Data e Hora'])
            feedbacks_df = feedbacks_df[feedbacks_df['Data e Hora'].dt.date == pd.Timestamp.now().date()]

            # Calcular total de clientes falados
            total_clientes_falados = len(feedbacks_df)

            # Calcular total de portas batidas
            total_portas_batidas = total_clientes_falados  # Cada linha representa uma porta batida

            # Calcular percentagens de clientes por fornecedor de energia
            percentagens_fornecedores = feedbacks_df['fornecedorEnergia'].value_counts(normalize=True) * 100

            # Calcular valores médios das faturas de energia por fornecedor
            valores_medios_faturas = feedbacks_df.groupby('fornecedorEnergia')['valorFaturaEnergia'].mean()

            # Calcular períodos horários de sucesso
            periodos_horarios_sucesso = feedbacks_df['Data e Hora'].dt.hour.value_counts().idxmax()

            # Convertendo valores int64 para int
            total_clientes_falados = int(total_clientes_falados)
            total_portas_batidas = int(total_portas_batidas)
            periodos_horarios_sucesso = int(periodos_horarios_sucesso)


            # Criar um novo DataFrame com os resultados
            resultados_df = pd.DataFrame({
                'Usuário': [logged_username],
                'Data': [datetime.now().strftime('%Y-%m-%d')],
                'Total Clientes Falados': [total_clientes_falados],
                'Total Portas Batidas': [total_portas_batidas],
                'Percentagens Fornecedores': [percentagens_fornecedores.to_dict()],
                'Valores Médios Faturas': [valores_medios_faturas.to_dict()],
                'Período Horário de Sucesso': [periodos_horarios_sucesso]
            })

            # Salvar os resultados em um arquivo Excel
            RESULTADOS_FOLDER = os.path.join(os.getcwd(), 'resultadosdia')
            if not os.path.exists(RESULTADOS_FOLDER):
                os.makedirs(RESULTADOS_FOLDER)
            resultados_file = os.path.join(RESULTADOS_FOLDER, f'resultadodia_{logged_username}_{datetime.now().strftime("%Y-%m-%d")}.xlsx')
            resultados_df.to_excel(resultados_file, index=False)

            # Preparar os resultados da análise
            analysis_results = {
            ...
            return jsonify({'error': 'Usuário não logado.'})
    except Exception as e:
...
@app.route('/obj_menu_main')
def obj_menu_main():
    return render_template('obj_menu_main.html')



@app.route('/set_objectives', methods=['GET', 'POST'])
def set_objectives():
    try:
        if request.method == 'POST':
            # Obter os dados do formulário
            gestor_comercial = request.form.get('gestor_comercial')
            obj_ativos_mes = request.form.get('obj_ativos_mes')
            obj_fechos_mes = request.form.get('obj_fechos_mes')
            obj_quest_validos_dia = request.form.get('obj_quest_validos_dia')
            obj_quest_validos_semana = request.form.get('obj_quest_validos_semana')
            obj_quest_validos_mes = request.form.get('obj_quest_validos_mes')

            # Salvar os dados em um arquivo XLSX
            objectives_df = pd.DataFrame({
                'GestorComercial': [gestor_comercial],
                'ObjAtivosMes': [obj_ativos_mes],
                'ObjFechosMes': [obj_fechos_mes],
                'ObjQuestValidosDia': [obj_quest_validos_dia],
                'ObjQuestValidosSemana': [obj_quest_validos_semana],
                'ObjQuestValidosMes': [obj_quest_validos_mes]
            })

            OBJECTIVES_FOLDER = os.path.join(os.getcwd(), 'objetivos')
            if not os.path.exists(OBJECTIVES_FOLDER):
                os.makedirs(OBJECTIVES_FOLDER)

            filename = f"{gestor_comercial}_objetivos_fixos.xlsx"
            objectives_df.to_excel(os.path.join(OBJECTIVES_FOLDER, filename), index=False)

            return 'Objetivos salvos com sucesso!'

        else:
            # Carregar o arquivo users_role.xlsx
            LOGS_FOLDER = os.path.join(os.getcwd(), 'logs')
            users_role_df = pd.read_excel(os.path.join(LOGS_FOLDER, 'users_role.xlsx'))

            # Filtrar os usuários com função de Gestor Comercial ou Gestor de Equipe
            gestores = users_role_df[(users_role_df['Role'] == 'GestorComercial') | 
                                    (users_role_df['Role'] == 'GestorEquipa')]

            # Obter lista de usuários para a caixa de seleção
            usuarios_gestores = gestores['Username'].tolist()

            return render_template('set_objectives.html', usuarios_gestores=usuarios_gestores)

    except Exception as e:
        return str(e)
    

@app.route('/carregar_results')
def carregar_results():
    return render_template('carregar_results.html')
